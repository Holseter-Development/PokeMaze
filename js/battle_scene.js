...
const BattleScene = {
    ...
    this.el.innerHTML = `
      <div class="bui">
        <div class="scene">
          ...
          <div class="sprites">
            <img id="pSprite" src="${player.back_sprite}" style="left:8%; bottom:10%; width:32%;">
            <img id="eSprite" src="${enemy.sprite}" style="right:8%; top:12%; width:28%;">
          </div>
          <div class="bars">
        ...
        <div class="menu">
          <div id="moves"></div>
          <div class="bagrun">
            <button id="btnCatch" class="btn-wide">Throw Poké Ball</button>
            <button id="btnRun" class="btn-wide">Run</button>
          </div>
  ...
  updateHP(player, enemy){
    const p = this.el.querySelector('#pHP'); if(p) p.style.width = Math.max(0, Math.round(100*player.hp/player.maxhp)) + '%';
    const e = this.el.querySelector('#eHP'); if(e) e.style.width = Math.max(0, Math.round(100*enemy.hp/ enemy.maxhp)) + '%';
  },
  say(text){
    const tb = this.el.querySelector('#textbox'); if(tb) tb.textContent = text;
  },
  hide(){
